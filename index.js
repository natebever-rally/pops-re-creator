...
const getReleases = async function(header, projectUuid) {
    ...
    const url = encodeURI(`${basePath}/release?project=/project/${projectUuid}&projectScopeDown=false&fetch=Name,ReleaseDate,ReleaseStartDate,ObjectID`);
...
const getItemTree = async function(header, projectUuid, types) {
    ...
    const url = encodeURI(`${basePath}/artifacts?types=PortfolioItem/${types[portfolioType].Name}&project=/project/${projectUuid}&projectScopeDown=true&projectScopeUp=false&pageSize=2000&fetch=Children,Name,RefinedEstimate,PreliminaryEstimate,Description,ObjectID,FormattedID,UserStories`);
